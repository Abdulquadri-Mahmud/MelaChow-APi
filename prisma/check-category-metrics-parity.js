...
const invoke = async (provider) => {
  process.env.DB_CATEGORY_METRICS_READ_PROVIDER = provider;
  let statusCode = 200;
  let payload = null;
  const res = {
    status(code) {
...
const main = async () => {
  ...
  try {
          ...
          results: [
            {
              ...
              counts: {
                mongo: mongoResponse.payload?.distribution?.length || 0,
                postgres: postgresResponse.payload?.distribution?.length || 0,
              },
            },
          ],
        },
    ...
    process.env.DB_CATEGORY_METRICS_READ_PROVIDER = "postgres";
...
main().catch(async (error) => {
  console.error("Category metrics parity check failed:", error);
