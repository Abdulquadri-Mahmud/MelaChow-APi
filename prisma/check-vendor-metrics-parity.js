...
const invoke = async (provider) => {
  process.env.DB_VENDOR_METRICS_READ_PROVIDER = provider;
  let statusCode = 200;
  let payload = null;
  const res = {
    status(code) {
...
const main = async () => {
  ...
  try {
          ...
          results: [
            {
              ...
              sales: {
                mongo: mongoResponse.payload?.data?.sales?.monthly || null,
                postgres: postgresResponse.payload?.data?.sales?.monthly || null,
              },
              registrations: {
                mongo: mongoResponse.payload?.data?.registrations?.monthly ?? null,
                postgres: postgresResponse.payload?.data?.registrations?.monthly ?? null,
              },
            },
          ],
        },
    ...
    process.env.DB_VENDOR_METRICS_READ_PROVIDER = "postgres";
...
main().catch(async (error) => {
  console.error("Vendor metrics parity check failed:", error);
