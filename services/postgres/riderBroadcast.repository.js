...
export const riderBroadcastRepository = {
  ...
  async offer(vendorOrderToken,{assignedBy=null}={}) {
    ...
    const candidates=await prisma.rider.findMany({where:{cityId,stateId,status:{in:["available","pending_assignment","on_delivery"]},isActive:true,isVerified:true,deletedAt:null}});
    ...
    await prisma.$transaction(async(tx)=>{
      ...
      for(const rider of riders){if(rider.status==="available")await tx.rider.update({where:{id:rider.id},data:{status:"pending_assignment",assignmentExpiresAt:expiresAt}});await tx.riderAssignment.create({data:{orderId:order.id,vendorOrderId,riderId:rider.id,vendorId:vendorOrder.restaurantId,stateId,cityId,status:"pending",expiresAt,metadata:{assignedBy,assignedAt:new Date().toISOString(),restaurantName:vendorOrder.restaurant.storeName,orderReadableId:order.orderCode,assignmentMode:"automatic"}}});}
